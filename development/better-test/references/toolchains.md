...
| Web 前端 | browser-dev skill（日常调试）；Playwright（回归套件） | browser-dev 已有登录态快照，单次验证优先用它 |
...
| macOS 原生 app | XCUITest；accessibility API 驱动 | 无测试链时用 computer-use skill（Orca）做取证兑底：accessibility 结构化读控件 + 操作 + 截图，比 AppleScript 盲坐标可靠一个档次；但它操作真实桌面，只算兑底不算达标链 |

## 反模式（见到即判不达标）

- **抢占用户输入的系统脚本**：AppleScript/cliclick/robotjs 盲坐标移动真实鼠标——不可并行、不可无头、打断用户工作（computer-use 的区别：结构化控件定位非盲坐标，可作兑底取证，但同样占用桌面，不能当常规测试链）
