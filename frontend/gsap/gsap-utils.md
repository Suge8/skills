...
```
### shuffle(array)
原地打乱数组并返回同一数组。用于随机化顺序（例如，从“随机”与副本错开）。
```javascript
...
| `amount` |number |分布在所有目标上的总计（添加到基础）。例如。 `amount: 1` 有 100 个目标（默认线性、从索引 0 开始）→ 相邻间隔约为 `1 / 99`。使用 **each** 来为每个目标设置固定步骤。 |
| `each` |number |每个目标之间添加的量（添加到基础）。例如。 `each: 1` 有 4 个目标 → 0、1、2、3。使用 **amount** 来分割总数。 |
| `from` |number\|string\|Array|分配开始的位置：索引，或 `"start"`、`"center"`、`"edges"`、`"random"`、`"end"` 或类似 `[0.25, 0.75]` 的比率。默认 `0`。 |
| `grid` |string\|Array|使用网格位置而不是平面索引：`[rows, columns]`（例如`[5, 10]`）或`"auto"`进行检测。对于平面数组省略。 |
| `axis` |string|对于网格：限制为一个轴（`"x"` 或 `"y"`）。 |
| `ease` |string\|Function|沿着缓动曲线分布值（例如 `"power1.inOut"`）。默认 `"none"`。 |
...
```
### unitize(func, unit?)
包装一个数值处理函数：先解析输入数字，再给返回值附加指定单位；省略单位时沿用输入单位。
```javascript
gsap.utils.unitize((value) => value, "px")(100); // "100px"
gsap.utils.unitize((value) => value)("2rem"); // "2rem"
```
...
## 不要
- ❌ 假设 **mapRange** / **normalize** 手柄单位；他们研究数字。当单位很重要时，使用 **getUnit** / **unitize**。
