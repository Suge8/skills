...
- **`docs/PRODUCT.md`**（战略）：记录界面类型、目标用户、产品目的、品牌性格、反例和设计原则，回答“给谁、做什么、为什么”。
- **`docs/DESIGN.md`**（视觉）：记录主题、配色、字体、组件和布局，遵循 [DESIGN.md 格式规范](https://raw.githubusercontent.com/google-labs-code/design.md/main/docs/spec.md)，回答“长什么样”。
...
检查现有状态。优先读取 `docs/PRODUCT.md` 和 `docs/DESIGN.md`；为兼容旧项目，也识别项目根目录或 `.agents/context/` 中的同名文件（不区分大小写）。同时检查 `.impeccable/live/config.json` 是否存在；若存在，步骤 6 不得覆盖。
...
写入 `PROJECT_ROOT/docs/PRODUCT.md`，按需创建 `docs/`。若兼容位置已有 PRODUCT.md，更新该现有文件，不得另建第二份事实源。
...
如果 init 是由另一个 Impeccable 命令触发的前置步骤，完成后恢复原任务。 Your own writes are the freshest source; no reload needed.
