生成 `docs/DESIGN.md`，记录当前视觉设计系统，使后续 Agent 生成的新界面保持品牌一致。
...
写入 `PROJECT_ROOT/docs/DESIGN.md`，按需创建 `docs/`。若兼容位置已有 DESIGN.md，更新该现有文件，不得另建第二份事实源。
...
每次重新生成 `docs/DESIGN.md` 时，同时重新生成 sidecar。 If the user only asks to refresh the sidecar (e.g., from the live panel's stale-hint), preserve `DESIGN.md` and write only `.impeccable/design.json`.
