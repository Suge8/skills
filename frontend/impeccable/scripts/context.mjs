 ...
 *   1. Active project docs/, then .agents/context/
 *   2. Active project root, for upstream compatibility
...
const FALLBACK_DIRS = ['docs', '.agents/context'];
...
function resolveLocalContextDir(root) {
  ...
  return firstExisting(root, [...PRODUCT_NAMES, ...DESIGN_NAMES]) ? root : null;
}
...
async function cli() {
  ...
  if (!ctx.hasProduct) {
    ...
    const parts = [
      ...
      'For a new build, craft, or shape flow, load reference/init.md first. ' +
      'For a scoped change to an existing interface, use the codebase as context and continue.',
    ];
