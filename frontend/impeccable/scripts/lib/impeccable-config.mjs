/**
 * Reader/writer for detector settings in the unified `.impeccable` config.
 * Shared settings live in config.json; per-developer overrides live in the
...
export function readDetectionConfig(root) {
  ...
  for (const filePath of [getConfigPath(root), getLocalConfigPath(root)]) {
    applyDetectionConfigSource(config, detectorSection(safeReadJson(filePath)));
  }
  return config;
}
