...
Rendered-pair contrast measurement and color remediation are covered by `reference/colors/`; visual text sizing and iOS input zoom by `reference/typography/`; spatial RTL layout by `SKILL.md` › 排版与间距.
...
| `maximum-scale=1` to stop iOS input zoom | 16px input font on mobile (see `reference/typography/`); never block zoom |
