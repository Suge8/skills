...
| 项目测试不行/缺测试链，或想让测试更好 | **"跑 better-test"** | better-test |
| agent 说修好了但你存疑 | 用你的原话："你确定修好了吗/真实测试了吗" | better-test |
| 问"还有优化空间吗"（验收干净 / 功能提升都算） | **"跑 optimize"**，它会分流减法/加法 | optimize |
...
1. "还有优化空间吗" → 直接说 **"跑 optimize"**——它会问你要减法（验收干净）还是加法（对标提升），两个分支都有终止条件，都敢说"没有了"
...
- **验证**：better-test 的养链部分是自动的（系统层强制：改动收工前确认有覆盖，没有就补或取证）。你只做验收：完成报告带 ✅ 证据就信；带 ⚠️ 未验证就照步骤花 30 秒验；啥都不带就说"你确定修好了吗"——这句老话就是触发器
- **工具**：browser-dev（网页调试）、computer-use（桌面操控，也是桌面 app 取证兑底）、ssh、web-clone（仿站）、gsap（动画）、logo、gpt-image、stop-slop（去 AI 味文案）、search-skills（翻归档技能）
- **归档里留着备用的**：office 五件套（docx/pdf/pptx/xlsx/obsidian，交作业时说"用归档的 pptx"）、其余已被现有 skill 覆盖不用碰

## 组合套路

- **新项目一条龙**：project-setup → grill 拷问需求 → prototype 挑方案 → 开发 → optimize 验收 → ship
- **功能升级一条龙**：optimize 加法分支找差距 → 你挑 →（大改先 prototype）→ 开发 → optimize 减法验收 → ship
- **修 bug 一条龙**：diagnose 定位 → 修 → 回归用例入链（better-test）→ ship
- **开源前**：project-setup（体检出缺的开源件）→ improve（全仓安全/依赖审计）→ optimize → ship
